...
- [ifopt](https://index.ros.org/p/ifopt/) (ROS package for IPOPT optimization)
- [rosbag2_cpp](https://index.ros.org/p/rosbag2_cpp/) (ROS package for saving ROS topic data)
...
```sh
mkdir -p ~/calibration_imu_ws/src/
cd ~/calibration_imu_ws
git clone https://github.com/pcdangio/ros-calibration_imu.git src/calibration_imu
colcon build
```
...
```sh
. install/setup.bash
ros2 run calibration_imu <node_name>
```
...
```sh
ros2 run calibration_imu magnetometer
```
...
3. Set up the node to subscribe to your platform's ROS topic for magnetometer data. The magnetometer node automatically subscribes to the `/magnetometer_broadcaster/magnetic_field` topic, which may be [remapped](http://wiki.ros.org/Remapping%20Arguments) when starting the magnetometer node. **CRITICAL:** Ensure that the magnetometer is transmitting uncalibrated data through the above topic.
...
```sh
ros2 run calibration_imu accelerometer
```
...
1. Set up the node to subscribe to your platform's ROS topic for raw **(not transformed)** accelerometer data. The accelerometer node automatically subscribes to the `/calibration_accelerometer/imu` topic, which may be [remapped](http://wiki.ros.org/Remapping%20Arguments) when starting the accelerometer node. **CRITICAL:** Ensure that the accelerometer is transmitting uncalibrated data through the above topic.
